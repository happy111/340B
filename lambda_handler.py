...
from utils.utils_helper import (create_error_response, validate_optional_param,
                                validate_anomalous_transactions_time_period,build_no_data_response,
                                build_anomalous_transactions_chart_data,validate_confidence_accounts_query_params,
                                build_confidence_accounts_donut_data,get_curr_prev_timeperiod,
                                build_score_growth_response,validate_growth_query_params,
                                build_growth_donut_data,map_row_to_response,
                                process_account_kpis,build_account_kpi_response,
                                build_account_detail_anomaly,build_account_detail_anomaly_response,
...
def get_anomalous_transactions_data(query_params: dict = None):
    ...
    try:
        ...
        error_response = validate_anomalous_transactions_time_period(
            query_params,
            tile_name
        )
        if error_response:
        ...
        if query_params:
            time_period = query_params.get(
                "time-period",
                "quarterly"
            )

            filter_string = generate_global_filters_from_query_params(
                query_params,
                valid_filter_column_map
            )
        ...
        if not result:
            logger.warning(
                f"No data returned from vwAnomaliesTransactions for "
                f"{tile_name} with filters: {query_params}"
            )
        ...
        chart_data = build_anomalous_transactions_chart_data(result)
...
def get_anomalies_confidence_accounts_data(query_params: dict = None):
    ...
    try:
        ...
        # Validate query parameters
        error_response = validate_confidence_accounts_query_params(
            query_params,
            tile_name
        )
        if error_response:
        ...
        donut_data = build_confidence_accounts_donut_data(result)
...
def get_anomalies_score_growth_data(query_params: dict = None):
    ...
    try:
        ...
        if query_params:
            ...
            if time_period:
                max_date = get_date_for_time_period(
                    time_period if query_params and query_params.get('time-period')
                    else 'quarterly'
                )

                curr_period, prev_period, error_response = get_curr_prev_timeperiod(
                    max_date,
                    time_period,
                    tile_name
                )
        ...
        return build_score_growth_response(
            result,
            curr_period,
            prev_period
        )
...
def get_anomalies_confidence_growth_data(query_params: dict = None):
    ...
    try:
        # Validate query parameters
        error_response = validate_growth_query_params(
            query_params,
            tile_name
        )
        if error_response:
        ...
        # Format response as donut chart data
        donut_data = build_growth_donut_data(result)
...
def get_anomalous_transaction_map_chargeback_data(query_params: dict = None):
    ...
    try:
        ...
        # Map view columns to response format
        map_data = [map_row_to_response(row) for row in result]
...
def get_account_detail_kpis_data(query_params: dict = None):
    ...
    try:
        ...
        result, current_period, previous_period, count, no_data_str = process_account_kpis(
            template,
            query_params,
            session,
            tile_name,
        ...
        return build_account_kpi_response(
            result,
            current_period,
            previous_period,
            no_data_str,
        ...
        return _classify_database_error(
            e,
            tile_name,
            "vwAnomalousTransactions /  340B_ProcessedData",
            is_view=True
        )
...
def get_account_detail_anomalies_data(query_params: dict = None):
    ...
    try:
        ...
        # Query the view with appropriate ID column based on id_type
        # query = get_account_detail_query(id_type)
        if id_type == "340B":
            query = text("""
                SELECT
        ...
        anomalies = [
            build_account_detail_anomaly(row)
            for row in result
        ]
...
def get_account_detail_anomalies_overall_data(query_params: dict = None):
    ...
    try:
        ...
        # query = get_account_detail_overall_query(id_type)
        if id_type == "340B":
            query = text("""
                SELECT
                    anomalyId, accountId, pharmacyId, anomalyEntityName,
        ...
        anomalies = [
            build_account_detail_anomaly_response(row)
            for row in result
        ]
        ...
        return _classify_database_error(
            e,
            tile_name,
            "vwAccountDetailAnomaliesOverall",
            is_view=True
        )
...
def get_tiles_bundle():
    ...
    try:
        ...
        #Extract tiles
        requested_tiles,error = get_requested_tiles(query_params)
        ...
        #Validate tiles
        error = validate_tiles(requested_tiles)
        if error:
            return error

        #Extract additional params
        additional_params = get_additional_params(query_params)
        ...
        return create_error_response(INTERNAL_SERVER_ERROR, str(e))
